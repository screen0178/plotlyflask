...
df_init = df[['ig_username','comment_count']].groupby(['ig_username'], sort=False).sum().reset_index()
df_init.sort_values('comment_count', axis = 0, ascending = False, 
                 inplace = True, na_position ='last') 
# print(df_init[:25])


def init_commentDashboard(server):
...
def init_callbacks(app):
    ...
    def update_output(value,value_month, start_date, end_date):
        if value == 'dinamis':
            ...
            dff = dff[['ig_username','comment_count']].groupby(['ig_username'], sort=False).sum().reset_index()
            dff.sort_values('comment_count', axis = 0, ascending = False, 
                                inplace = True, na_position ='last')
            # print(dff)
            dff = dff[0:25]
            fig = {
                    ...
                    'layout': {
                        'title': 'Total comment from {} until {} per account'.format(start_date,end_date),
                        'height': 500,
                        'padding': 150
                    }
                }
            ...
            dff = df.groupby(['ig_username',pd.Grouper(key='taken_at')])['comment_count'].sum().reset_index()
            ...
            dff = df[['ig_username','comment_count','taken_at']]
            dff['month'] = dff['taken_at'].dt.month
            dff['month'] = dff['month'].astype(str)
            dff = dff[['ig_username','comment_count','month']].groupby(['ig_username','month'], sort=True).sum().reset_index()
