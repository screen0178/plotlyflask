...
df_init = df[['ig_username','url']].groupby(['ig_username'], sort=False).count().reset_index()
df_init.sort_values('url', axis = 0, ascending = False, 
                 inplace = True, na_position ='last') 
# print(df_init[:25])


def init_postDashboard(server):
    """Create a Plotly Dash dashboard."""
    dash_app = dash.Dash(
        server=server,
        routes_pathname_prefix='/postDashboard/',
...
def init_callbacks(app):
    ...
    def update_output(value,value_month, start_date, end_date):
        if value == 'dinamis':
            ...
            dff = dff[['ig_username','url']].groupby(['ig_username'], sort=False).count().reset_index()
            dff.sort_values('url', axis = 0, ascending = False, 
                                inplace = True, na_position ='last')
            ...
            dff = dff[['ig_username','url','month']].groupby(['ig_username','month'], sort=True).count().reset_index()
